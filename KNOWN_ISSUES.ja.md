...
- 意図する修正: リクエスト時のポインタ lookup が null を返したら
  エントリを削除 + 小さな LRU 上限。注意: 削除してよいのは null の
  ときだけ。lookup が throw した場合は state store に到達できない
  ということであり、そのエントリこそ chat proxy の fail-open 経路が
  配信する当のもの (同ファイルの `failOpen`)。ここで削除すると
  state store 障害のたびにデータ経路が再び壊れる。
