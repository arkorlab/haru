...
| `HARU_CHAT_HEADER_TIMEOUT_MS` | chat proxy の TTFB 上限 (デフォルト 30000)。**非ストリーミング**の長い補完ではレスポンスヘッダーが生成完了後にしか届かないため、必要に応じて引き上げてください。約 300000 を超える値は Node fetch 自身の headersTimeout で頭打ちになります (KNOWN_ISSUES 参照)。 |
