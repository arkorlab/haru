...
送ってください。未設定は未認証を意味します (ローカル開発のみ。
サーバーは大きな警告をログに出します)。サーバーとスーパーバイザーの
間は別の `HARU_SUPERVISOR_TOKEN` を使います。
...
| `HARU_API_TOKEN` | 公開 API の Bearer トークン。未設定 = オープン (開発のみ)。 |
| `HARU_SUPERVISOR_TOKEN` | ドメインのスーパーバイザーへ提示する Bearer トークン。 |
| `HARU_DEFAULT_FLEET` | `X-Haru-Fleet` ヘッダーなしの `/v1/chat/completions` が使うフリート。 |
| `HARU_CHAT_HEADER_TIMEOUT_MS` | chat proxy の TTFB 上限 (デフォルト 30000)。**非ストリーミング**の長い補完ではレスポンスヘッダーが生成完了後にしか届かないため、必要に応じて引き上げてください。 |
...
コントリビューター向けの先送り作業 (構造的負債、効率の改善
バックログ、テスト妥当性のメモ) は、ファイル参照と修正方針付きで
[KNOWN_ISSUES.ja.md](KNOWN_ISSUES.ja.md) に記録しています。

- **自動フェイルオーバーはハートビートの鮮度切れ、または `failed`
  状態のドメインでのみ発火します。** スーパーバイザーが生きたまま
  vLLM プロセスだけが死んだ Active ドメインは `degraded` になります
  (route intent の適格性に反映されます) が、自動昇格の対象には
  なりません。手動で昇格するか、route intent を外部で消費して
  対処してください。
...
- **レイアウトの再適用は既存行を更新しません** (フリートポリシー、
  ...
  **新規追加**スロットの状態はライブなルーティングポインターに
  従います。
- **chat ルーティングは昇格をスナップショットキャッシュ TTL (2 秒)
  まで遅れて反映することがあります。** chat proxy はフリートごとの
  スナップショットキャッシュから配信します。そのウィンドウ中に
  降格中の旧 active に当たったリクエストは即時に失敗するので、
  リトライしてください。これは意図的な上限です (理由と修正方針は
  KNOWN_ISSUES を参照)。
