...
- **合成プローブのポリシーには上限があります。**
  `policy.probe.prompt` は JSON Schema の `maxLength` と同じ Unicode
  コードポイント数で最大 8,192、`policy.probe.maxTokens` は最大 256
  です。DB schema は `db:push` でも両方の上限を強制します。
  migration `0003` もアップグレード前に保存済みフリートポリシーを
  検査し、どちらかの上限を超える既存値を切り詰めずに停止します。
  アップグレード前に該当する保存済みキーを短縮、引き下げ、または
  削除してください。レイアウトを再適用しても既存のポリシー行は
  更新されません。
